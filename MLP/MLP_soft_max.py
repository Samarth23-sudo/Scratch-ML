...
class MLPClassifier:
    ...
    def fit(self, X, y_one_hot,X_val=None, y_val=None):
        ...
        # get batch size based on the optimizer and the update would be according to it 
    ...
    def gradient_check(self, X, y, epsilon=1e-4):
        ...
        for l in range(len(self.biases)):
            ...
            for i in range(self.biases[l].shape[1]):
                ...
                loss_plus_epsilon = np.sum((self.predict(X) - y) ** 2)
                
                # f(x - epsilon)
                self.biases[l][0, i] = original_value - epsilon
                loss_minus_epsilon = np.sum((self.predict(X) - y) ** 2)
