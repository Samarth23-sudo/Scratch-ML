...
class MLPClassifier_Regressor:
    def __init__(self, input_size, hidden_layers, output_size, learning_rate=0.01,
                 activation='relu', optimizer='sgd', batch_size=32, epochs=100,is_regression=False):
        ...
        self.optimizers = {
            'sgd': SGD(),
            'batchgd': BatchGD(),
            'minibatchgd': MiniBatchGD(batch_size)
        }
    ...
    def _forward(self, X):
        ...
        for i, (w, b) in enumerate(zip(self.weights, self.biases)):
            ...
            if self.is_regression and i == len(self.weights) - 1:
    ...
    def _backward(self, X, y, activations, pre_activations):
        ...
        if self.is_regression:
            delta = mse_loss_derivative(y, activations[-1]) #last layer is linear
        else:
            delta = mse_loss_derivative(y, activations[-1]) * self.activation_derivative(pre_activations[-1])
    ...
    def fit(self, X, y_one_hot,X_val=None, y_val=None):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, X.shape[0], batch_size_1):
                ...
                epoch_loss += mse_loss(y_batch, activations[-1])
                
            epoch_loss_list.append(epoch_loss/self.batch_size)
            
            train_predict = self.predict(X)
            train_acc = self.accuracy(X,y_one_hot)
            train_loss = mse_loss(y_one_hot,train_predict)
            # Log metrics
            # wandb.log({
            #     "epoch": epoch + 1,
            ...
            if X_val is not None and y_val is not None:
                ...
                val_rmse_loss = rmse_loss(y_val, val_predictions)
                val_r2_loss = r2_score(y_val, val_predictions)
                ...
                #     "val_r2_loss": val_r2_loss,
                #     "val_mae_loss":val_mae
                # })
                #print(f'Epoch {epoch + 1}, Validation Loss: {val_loss}, Accuracy: {val_acc}')
                
            print(f'Epoch {epoch + 1}, MSE Loss: {epoch_loss/self.batch_size}')
        ...
        for l in range(len(self.weights)):
            ...
            for i in range(self.weights[l].shape[0]):
                for j in range(self.weights[l].shape[1]):
                    ...
                    if self.is_regression:
                        loss_plus_epsilon = mse_loss(y,self.predict(X))
                    else:
                        loss_plus_epsilon = np.sum((self.predict(X) - y) ** 2)  # Compute loss with perturbed weight mse
                    
                    # f(x - epsilon)
                    self.weights[l][i, j] = original_value - epsilon
                    if self.is_regression:
                        loss_minus_epsilon = mse_loss(y,self.predict(X))  # Compute loss with perturbed weight
                    else:
                        loss_minus_epsilon = np.sum((self.predict(X) - y) ** 2)  # Compute loss with perturbed weight
