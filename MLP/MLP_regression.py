...
class MLPRegressor:
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        for epoch in range(self.epochs):
            ...
            train_loss = mse_loss(y,train_predict)
            ...
            if X_val is not None and y_val is not None:
                ...
                print(f'Epoch {epoch + 1}, Validation MSE Loss: {val_loss}')
                print(f'Epoch {epoch + 1}, Validation RMSE Loss: {val_rmse_loss}')
                print(f'Epoch {epoch + 1}, Validation R2 score: {val_r2_loss}')
                print(f'Epoch {epoch + 1}, Validation MAE score: {val_mae}')
    ...
    def gradient_check(self, X, y, epsilon=1e-4):
        ...
        for l in range(len(self.weights)):
            ...
            for i in range(self.weights[l].shape[0]):
                for j in range(self.weights[l].shape[1]):
                    ...
                    loss_plus_epsilon = mse_loss(y,self.predict(X))  # Compute loss with perturbed weight mse
        ...
        for l in range(len(self.biases)):
            ...
            for i in range(self.biases[l].shape[1]):
                ...
                # f(x - epsilon)
                self.biases[l][0, i] = original_value - epsilon
                loss_minus_epsilon = mse_loss(y,self.predict(X))
